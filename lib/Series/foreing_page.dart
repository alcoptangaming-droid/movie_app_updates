...
class SeriesForeignPage extends StatefulWidget {
  final Function(Movie, Duration) onEpisodeWatched;

  const SeriesForeignPage({super.key, required this.onEpisodeWatched});

  @override
  _SeriesForeignPageState createState() => _SeriesForeignPageState();
}

class _SeriesForeignPageState extends State<SeriesForeignPage> {
  ...
  void _startAutoRefresh() {
    _refreshTimer = Timer.periodic(const Duration(minutes: 10), (timer) {
      if (!_showSearchBar && mounted) {
        print("Auto-refreshing foreign series list in SeriesForeignPage...");
  ...
  Future<void> loadSeries({bool isManualRefresh = true}) async {
    ...
    if (initialLoad && mounted) {
      ...
      print("Refreshing foreign series list...");
    }

    try {
      final loadedSeries = await ApiService.getForeignSeries();
      if (mounted) {
        setState(() {
      ...
      print('[SeriesForeignPage] Error loading series: $e');
      if (mounted) {
    ...
    Future.microtask(() {
      ...
      final results = seriesList.where((series) {
        return series.title.toLowerCase().contains(lowerCaseQuery) ||
            (series.description?.toLowerCase().contains(lowerCaseQuery) ??
                false);
      }).toList();
              ...
              child: Center(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 16),
                    const Text('لا توجد مسلسلات أجنبية متاحة حالياً',
                        style: TextStyle(fontSize: 18, color: Colors.grey)),
