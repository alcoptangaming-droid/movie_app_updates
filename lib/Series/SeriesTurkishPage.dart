...
class SeriesTurkishPage extends StatefulWidget {
  final Function(Movie, Duration) onEpisodeWatched;

  const SeriesTurkishPage({super.key, required this.onEpisodeWatched});

  @override
  _SeriesTurkishPageState createState() => _SeriesTurkishPageState();
}

class _SeriesTurkishPageState extends State<SeriesTurkishPage> {
  ...
  void _startAutoRefresh() {
    _refreshTimer = Timer.periodic(const Duration(minutes: 10), (timer) {
      if (!_showSearchBar && mounted) {
        print("Auto-refreshing Turkish series list...");
  ...
  Future<void> loadSeries({bool isManualRefresh = true}) async {
    ...
    if (initialLoad && mounted) {
      ...
      print("Refreshing Turkish series list...");
    }

    try {
      final loadedSeries = await ApiService.getTurkishSeries();
      if (mounted) {
        setState(() {
      ...
      print('[SeriesTurkishPage] Error loading Turkish series: $e');
      if (mounted) {
    ...
    return Container(
      ...
      child: TextField(
        ...
        decoration: const InputDecoration(
          hintText: 'ابحث في المسلسلات التركية...',
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const Text('جاري تحميل المسلسلات التركية...',
              ...
              child: Center(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 16),
                    const Text('لا توجد مسلسلات تركية متاحة حالياً',
                        style: TextStyle(fontSize: 18, color: Colors.grey)),
