...
class MoviesForeignPage extends StatefulWidget {
  const MoviesForeignPage({super.key});

  @override
  _MoviesForeignPageState createState() => _MoviesForeignPageState();
}

class _MoviesForeignPageState extends State<MoviesForeignPage> {
  List<Movie> movies = [];
  bool isLoading = true;
  ...
  Future<void> _openVideoPlayer(Movie movie) async {
    ...
    Duration finalPosition = startAt;
  ...
  Future<void> _updateWatchHistoryInPrefs(
    ...
    try {
      final List<String> listToSaveJson =
          currentList.take(10).map((m) => json.encode(m.toJson())).toList();
      await prefs.setStringList('continue_watching_list', listToSaveJson);
    } catch (e) {
  ...
  Future<void> loadMovies({bool showMessages = true}) async {
    ...
    try {
      // ✅ استخدام الأفلام الأجنبية بدلاً من العربية
      final loadedMovies = await ApiService.getForeignMovies();
      ...
      if (mounted) {
        ...
        if (showMessages) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: const Text('تم تحديث قائمة الأفلام الأجنبية!'),
      ...
      print('[MoviesForeignPage] Error loading movies: $e');
      if (mounted) {
    ...
    Future.microtask(() {
      ...
      final results = movies.where((movie) {
        return movie.title.toLowerCase().contains(lowerQuery) ||
            ...
            (movie.category?.toLowerCase().contains(lowerQuery) ?? false);
      }).toList();
  ...
  AppBar _buildMainAppBar() {
    return AppBar(
      ...
      title: _showSearchBar
          ? _buildSearchField()
          : const Text('أفلام أجنبية', // ✅ تغيير العنوان
              style: TextStyle(
                color: Colors.white,
    ...
    return Container(
      ...
      child: TextField(
        ...
        decoration: const InputDecoration(
          hintText: 'ابحث في الأفلام الأجنبية...', // ✅ تغيير النص
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const Text('جاري تحميل الأفلام الأجنبية...', // ✅ تغيير النص
              ...
              child: Center(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 16),
                    const Text('لا توجد أفلام أجنبية متاحة حالياً', // ✅ تغيير النص
                        style: TextStyle(fontSize: 18, color: Colors.grey)),
                  ...
                  children: [
                    ...
                    FutureBuilder<SharedPreferences>(
                      ...
                      builder: (context, snap) {
                        ...
                        if (progressPercent <= 0 || progressPercent > 0.95) return const SizedBox.shrink();
